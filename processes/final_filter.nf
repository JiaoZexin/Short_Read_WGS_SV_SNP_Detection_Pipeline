process final_filter {
    ...
    input:
    path merged_vcf from file("8.duphold/*.smoove.square.vcf.gz")
    ...
    # Step 4: gap region 
    perl -ne 'chomp;if(/^>(\S+)/){$head=$1;$i=0;next};@a=split("",$_);foreach(@a){$i++;if($_ eq "N" && $s==0){$z=$i-1;print "$head	$z";$s=1}elsif($s==1 && $_ ne "N"){$j=$i-1;print "	$j
";$s=0}}' \
      2.index/ref.fa > 9.final/ref.gap.txt
    ...
    # Step 6: for bed
    awk '{OFS="	"; print $1,$2,$3,$4"_"$5}' 9.final/body.filtered > 9.final/body.bed.withid
    ...
    while read line; do echo $line | grep -o '\./\.' | wc -l; done < 9.final/body.bed.clean > 9.final/total_missing.txt
    while read line; do echo $line | grep -o 0/0 | wc -l; done < 9.final/body.bed.clean > 9.final/total_00.txt
    while read line; do echo $line | grep -o 0/1 | wc -l; done < 9.final/body.bed.clean > 9.final/total_01.txt
    while read line; do echo $line | grep -o 1/1 | wc -l; done < 9.final/body.bed.clean > 9.final/total_11.txt
